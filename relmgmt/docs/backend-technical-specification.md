...
- **Query Parameters**:
  - `from` (optional): Filter by start date (ISO yyyy-MM-dd)
  - `to` (optional): Filter by end date (ISO yyyy-MM-dd)
...
- **Response**: List of weekly over-allocation entries per resource

#### 5.9.4 Report Export Endpoints

- **Query variant**: `GET /api/v1/reports/export?type=ALLOCATION_CONFLICTS|RESOURCE_UTILIZATION|RELEASE_TIMELINE&...params`
- **Path variant**: `GET /api/v1/reports/{type}/export?...`
...
#### 5.9.5 Capacity Forecast Report

- **Endpoint**: `GET /api/v1/reports/capacity-forecast`
- **Query Parameters**:
  - `from` (optional): ISO date yyyy-MM-dd
  - `to` (optional): ISO date yyyy-MM-dd (must be >= from)
  - `skillFunction` (optional): Filter by skill function
  - `skillSubFunction` (optional): Filter by sub-function
- **Response**: Array of `CapacityForecastResponse`
  - `resourceId`, `resourceName`, `weekStarting`, `allocatedDays`, `capacity` (4.5), `availableDays`

#### 5.9.6 Skill-based Capacity Forecast Report
...
- **Query Parameters**:
  ...
  - `skillFunction` (optional): Filter by skill function
  - `skillSubFunction` (optional): Filter by sub-function
- **Response**: Array of `SkillCapacityForecastResponse`
  - `skillFunction`, `skillSubFunction`, `weekStarting`, `allocatedDays`, `capacity` (#active resources in skill * 4.5), `availableDays`

#### 5.9.7 DTOs

- `ResourceUtilizationRow`: `resourceId`, `resourceName`, `weekStarting`, `allocatedDays`, `capacity`, `utilizationPercent`
- `ReleaseTimelineRow`: `releaseId`, `name`, `startDate`, `endDate`
- `CapacityForecastResponse`: `resourceId`, `resourceName`, `weekStarting`, `allocatedDays`, `capacity`, `availableDays`
- `SkillCapacityForecastResponse`: `skillFunction`, `skillSubFunction`, `weekStarting`, `allocatedDays`, `capacity`, `availableDays`

#### 5.9.8 Validation & Errors

- Date ranges: if `to` < `from` respond `400 Bad Request`
- `resourceIds`: non-numeric values ignored in export; API accepts only numeric IDs

#### 5.9.9 Export

- **Query variant**: `GET /api/v1/reports/export?type=ALLOCATION_CONFLICTS|RESOURCE_UTILIZATION|RELEASE_TIMELINE|CAPACITY_FORECAST|SKILL_CAPACITY_FORECAST&...params`
- Summary sheet includes report type, generated timestamp, and filters. Data sheet includes report-specific formatted rows.
